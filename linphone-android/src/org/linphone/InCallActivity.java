...
import org.linphone.jifeng.service.PttService;
...
public class InCallActivity extends FragmentActivity implements
	...
	private TextView pause, hangUp, dialer, video, micro, speaker, options,
			addCall, transfer, conference;
		...
		mListener = new LinphoneCoreListenerBase() {
			...
			public void callState(LinphoneCore lc, final LinphoneCall call,
				...
				if (state == State.StreamsRunning) {
					...
					// FIXME //lh 非控制方账号且是会议就静音
					if (PttService.getInstance().isConfAndNotCtrl(lc)) {
						isMicMuted = true;
						LinphoneManager.getLc().muteMic(true);
//						micro.setBackgroundResource(R.drawable.micro_off);
					}
					//会议且是控制方，需要探测会议结束标示，结束通话
					if (PttService.getInstance().isConference(LinphoneManager.getLc().getCurrentCall())
							&& PttService.getInstance().isControl(LinphoneManager.getLc())) {
						isSpeakerEnabled = true;
						LinphoneManager.getLc().enableSpeaker(isSpeakerEnabled);
						if (!confEndDetect) {
							confEndDetect = true;
							startConfEndDetectAsyncTask().execute();
						}
					}
					//////////
				...
				if (state == State.CallUpdatedByRemote) {
					...
					boolean remoteVideo = call.getRemoteParams()
							.getVideoEnabled();
					boolean localVideo = call.getCurrentParamsCopy()
							.getVideoEnabled();
					boolean autoAcceptCameraPolicy = LinphonePreferences
							.instance()
			...
			@Override
			public void callEncryptionChanged(LinphoneCore lc,
					final LinphoneCall call, boolean encrypted,
		...
		if (findViewById(R.id.fragmentContainer) != null) {
			...
			if (LinphoneManager.getLc().getCallsNb() > 0) {
				...
				if (LinphoneUtils.isCallEstablished(call)) {
					// FIXME //lh 非控制方账号且是会议就静音
					if (PttService.getInstance().isConfAndNotCtrl(LinphoneManager.getLc())) {
						isMicMuted = true;
						LinphoneManager.getLc().muteMic(true);
//						micro.setBackgroundResource(R.drawable.micro_off);
					}
	...
	//FIXME //lh 会议结束线程探测
	boolean confEndDetect = false;
	private AsyncTask<String, Integer, Integer> startConfEndDetectAsyncTask() { 
		if (!confEndDetect) {
			return null;
		}
		return new AsyncTask<String, Integer, Integer>() {
			...
			protected Integer doInBackground(String... params) {
				...
				Integer ret = PttService.getInstance().confEndDetectReq(lc);
				return ret;
			}
			...
			protected void onPostExecute(Integer ret) {
				try {
					Random r = new Random();
					Thread.sleep(r.nextInt(1000) + 1500);
	...
	private void refreshInCallActions() {
		...
		if (LinphoneManager.getLc().getCallsNb() > 1) {
			...
			List<LinphoneCall> pausedCalls = LinphoneUtils.getCallsInState(
					LinphoneManager.getLc(), Arrays.asList(State.Paused));
	...
	// FIXME lh //非控制方处于会议模式需要poc功能
	private AsyncTask<Boolean, Integer, PttLootRet> getLootMicAsyncTask() { 
		return new AsyncTask<Boolean, Integer, PttLootRet>() {
	
			@Override
			protected PttLootRet doInBackground(Boolean... params) {
				LinphoneCore lc = LinphoneManager.getLc();
				PttLootRet ret = PttService.getInstance().lootMicReq(lc,
						params[0] ? true : false);
				return ret;
			}
	
			@Override
			protected void onPostExecute(PttLootRet ret) {
				if (null != ret && ret.isSuc() && ret.isOpenMic()) {
					openMicro();
				} else {
					if (null != ret && !ret.isSuc()) {
						Toast.makeText(InCallActivity.this, ret.getMsg(), Toast.LENGTH_SHORT).show();
					}
					closeMicro();
				}
			}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (id == R.id.micro) {
			LinphoneCore lc = LinphoneManager.getLc();
			if (PttService.getInstance().isConfAndNotCtrl(lc)) {
				if (event.getAction() == MotionEvent.ACTION_UP && isMicMuted == false) {
					getLootMicAsyncTask().execute(false);
				} else if (event.getAction() == MotionEvent.ACTION_DOWN) {
					getLootMicAsyncTask().execute(true);
				}
			}
		}
		return false;
	...
	public void onClick(View v) {
		...
		if (id == R.id.video) {
			enabledOrDisabledVideo(isVideoEnabled(LinphoneManager.getLc()
					.getCurrentCall()));
		} else if (id == R.id.micro) {
			// FIXME //lh //控制方或非会议，自己控制mic
			// toggleMicro();
			LinphoneCore lc = LinphoneManager.getLc();
			if (PttService.getInstance().isControl(lc)
					|| !PttService.getInstance().isConference(
							lc.getCurrentCall())) {
				toggleMicro();
			}
			// //////////
	...
	protected void onResume() {
		...
		if (lc != null) {
			...
			//会议且是控制方，需要探测会议结束标示，结束通话
//			if (PttService.getInstance().isConference(lc.getCurrentCall())
//					&& PttService.getInstance().isControl(lc)) {
//				isSpeakerEnabled = true;
//				LinphoneManager.getLc().enableSpeaker(isSpeakerEnabled);
//				startConfEndDetectAsyncTask().execute();
//			}
	...
	private void displayCall(Resources resources, LinphoneCall call, int index) {
		...
		Contact contact = ContactsManager.getInstance().findContactWithAddress(
				lAddress);
		if (contact != null) {
	...
	private void setContactName(LinearLayout callView,
		...
		Contact lContact = ContactsManager.getInstance()
				.findContactWithAddress(lAddress);
		if (lContact == null) {
